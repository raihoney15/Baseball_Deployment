class AfterOutService
  ...
  def call
    ...
    scoreboard = Scoreboard.create(balls: 0, hit: 0, run: 0, strike: 0, out: 0, event_id: @event.id, event_inning_id: @event.event_innings.last.id, home_team: false,home_away: true)

    team_line_ups = @event.team_line_ups
    first_base_rooster_id = team_line_ups.find_by(batter_order: 1)&.rooster_id

    rooster_position = RoosterPosition.new(scoreboard_id: @scoreboard.id, user_id: @current_user.id)
